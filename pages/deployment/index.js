...
const index = () => {
  ...
  // useEffect(() => {
  //   let todo = JSON.parse(localStorage.getItem("todos"))?.filter(({ desc }) => desc?.toLowerCase().includes('deployment'))
  //   setTodos(todo)
  // }, [])
  ...
  todos = todos?.filter(({ description }) => description?.toLowerCase().includes('deployment'))
